...
def load_pdf_documents(docs_dir: str = "data/docs"):
    ...
    for pdf_file in pdf_files:
        try:
            ...
            print(f"Erro ao abrir PDF {pdf_file}: {error}")
            continue
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            try:
                ...
                print(f"Erro ao extrair página {page_number} de {pdf_file}: {error}")
                continue
            ...
            try:
                source = str(pdf_file.relative_to(docs_path))
            except ValueError:
                source = str(pdf_file)
...
def chunk_documents(documents, chunk_size: int = 300, chunk_overlap: int = 50):
